...
int main(){
    ...
    cin>>n;
    arvore = vector<vector<int>> (n+1); valores = vector<ll> (n+1); depth = vector<int> (n+1,0);
    tin = vector<int> (n+1); tout = vector<int> (n+1); l = ceil(log2(n));
    up = vector<vector<int>> (n+1, vector<int> (l + 1));
    mxn = vector<vector<int>> (n+1, vector<int> (l + 1));
    int a, b;
    for(int i = 0; i<n-1; i++){
        cin>>a>>b;
    ...
    //cout<<(kth_anc(8,4))<<'\n';
    cin>>a>>b;
    int distancia = depth[a] + depth[b] - 2*depth[(lca(a,b))];
    cout<<"distancia de "<<a<<" e "<<b<<" = "<<distancia;
    return 0;
}