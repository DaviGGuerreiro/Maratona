...
vector<Query> aux;
vector<ll> s, p, esquerda, direita, esq, dir;
...
void dnc(int l, int r, int ql, int qr){
    ...
    for(int i = m_start; i<= m_end; i++){
        x = queries[i];
        a1 = s[x.l] + p[x.r] - vec[m];
        resposta[x.idx] = max(a1, esq[x.l]);
        resposta[x.idx] = max(resposta[x.idx], direita[x.r]);  //query x é uma query do meio que tem como menor valor ou o menor sufixo [l,m] ou o menor prefixo [m,r]
    }
...
void solve(){
    ...
    s = vector<ll> (n);
    p = vector<ll> (n);
    esquerda = vector<ll> (n);
    direita = vector<ll> (n);
    resposta = vector<ll> (q);
    esq = vector<ll> (n);
    dir = vector<ll> (n);
